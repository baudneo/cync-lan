...
# For device status packets index 32,59 is for off/on. on=1 off=0
...
# client sometimes sends packet with a timestamp in it (ts is not synced, like 2+ years behind)
...
# Unknown
UNKNOWN_BUFFER = bytearray([0x43, 0x00, 0x00, 0x00])
        ...
        try:
            while True:
                ...
                if data[0] == CLIENT_INFO_BUFFER:
                    ...
                    device.msgs[_e] = bytes2list(data)
                    continue
                ...
                # There is some sort of timestamp in the packet
                    ...
                    ts = data[ts_idx:]
                    logger.debug(
                        f"{lp} Client sent TIMESTAMP BUFFER -> {ts} - replying..."
                    )
                    await device.write(SERVER_CLIENT_DATA_ACK)
                    device.msgs[_e] = bytes2list(data)
                    continue
                ...
                # Newer firmware direct device status
                    ...
                    logger.debug(
                        f"{lp} Client sent DEVICE STATE, learning how to parse - replying "
                ...
                # unknown data
                elif data[0:4] == UNKNOWN_BUFFER and data[5] == 0x39:
                    ...
                    if data == b"C\x00\x00\x00\x079\x87\xa6\xd6\x01\x01\x06":
                        ...
                        logger.debug(
                            f"{lp} Client sent Unknown data with a unique header => {data[0:6]} - replying..."
                        )
                    await device.write(SERVER_CLIENT_DATA_ACK)
                # unknown data
                elif data[0:5] == CLIENT_DATA:
                    logger.debug(f"{lp} Client sent UNKNOWN DATA")
                    await device.write(SERVER_CLIENT_DATA_ACK)
                logger.debug(
                    f"{lp} Received {data_len} bytes in {(_e - _s):.5f} s\nRAW: {data}\nINT: "
                    f"{bytes2list(data)}\nHEX: {data.hex()}"
                )
