                ...
                if packet_data is not None:
                    ...
                    if packet_data[0] == 0x7E:
                        ...
                        if ctrl_bytes == bytes([0xF9, 0x52]):
                            ...
                            # 15th byte of inner struct is start of mesh info // JULY 2024 it's now 16th byte
                            # RAW MESH INFO //
                            # len 404 for 16th byte
                            # 1f 00 00 00 f9 52 7d 5e 00 05 00 00 00 05 00[14] 07 00 89 01 00 00 89 01 01 00 00 00 64 00 00 00 fe 00 00 00 10 00 f0 00 08 00 00 01 00 00 00 01 01 00 00 00 64 00 00 00 fe 00 00 00 f8 00 00 00 05 00 00 01 00 00 00 01 01 00 00 00 64 00 00 00 00 00 00 00 00 00 00 00 0a 00 00 01 00 00 00 01 01 00 00 00 5a 00 00 00 64 00 00 00 00 00 00 00 13 00 00 01 00 00 00 01 01 00 00 00 5a 00 00 00 64 00 00 00 00 00 00 00
                            # 1f 00 00 00 f9 52 de 00 09 00 00 00 09 00 09[14] 00 89
                            # inner_struct[14:38]=b'\x00\x07\x00\x89\x01\x00\x00\x89\x01\x01\x00\x00\x00d\x00\x00\x00\xfe\x00\x00\x00\x10\x00\xf0'
                            # inner_struct[38:62]=b'\x00\x08\x00\x00\x01\x00\x00\x00\x01\x01\x00\x00\x00d\x00\x00\x00\xfe\x00\x00\x00\xf8\x00\x00'
                            # inner_struct[62:86]=b'\x00\x05\x00\x00\x01\x00\x00\x00\x01\x01\x00\x00\x00d\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
                            # inner_struct[86:110]=b'\x00\n\x00\x00\x01\x00\x00\x00\x01\x01\x00\x00\x00Z\x00\x00\x00d\x00\x00\x00\x00\x00\x00'
                            # inner_struct[110:134]=b'\x00\x13\x00\x00\x01\x00\x00\x00\x01\x01\x00\x00\x00Z\x00\x00\x00d\x00\x00\x00\x00\x00\x00'
                            # inner_struct[134:158]=b'\x00'
                            minfo_start_idx = 14
                            if inner_struct[minfo_start_idx] == 0x00:
                                logger.debug(f"{lp}mesh: dev_id is 0 when using index: {minfo_start_idx}, "
                                             f"trying index 15: {inner_struct[15]}")
                                minfo_start_idx = 15

                            if inner_struct[minfo_start_idx] == 0x00:
                                logger.warning(f"{lp}mesh: dev_id is 0 when using index: {minfo_start_idx}, skipping...")
                            else:
                                self.mesh_info = None
                                # from what i've seen, after the first 14 bytes, the mesh info is 24 bytes long and repeats
                                # until the end.
                                # JULY 2024: ALl of a sudden it is now byte # 16 (index: 15) ???
